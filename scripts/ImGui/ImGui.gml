function ImGui() constructor {
	...
	/// @context ImGui
	/// @return {Undefined}
	static ShowHelpMarker = function(desc = "")
	{
		ImGui.SameLine();
		ImGui.TextDisabled("(?)");
	...
	/// @context ImGui
	/// @return {Undefined}
	static ShowTooltip = function(desc = "")
	{
	    if (ImGui.IsItemHovered() && SETTINGS.showTooltips) {
	        ImGui.BeginTooltip();
	        ImGui.PushTextWrapPos(450.0);
	        ImGui.TextUnformatted(desc);
	        ImGui.PopTextWrapPos();
	...
	/// @context ImGui
	/// @return {Undefined}
	static InputTextCustom = function(title, val, label = "##hidden", space = 100, def = NULL, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		...
		// Input Text
		var ret = ImGui.InputText(label, (val != NULL) ? val : "", flags);
		if (ret == "") ret = NULL;
	...
	/// @context ImGui
	/// @return {Undefined}
	static CheckboxCustom = function(title, val, label = "##hidden", space = 100)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
	...
	/// @context ImGui
	/// @return {Undefined}
	static ColourEditCustom = function(title, val, label = "##hidden", space = 100, def = NO_DEFAULT, flags = ImGuiColorEditFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
		...
		// Input Text
		var ret = ImGui.ColorEdit4(label, val, flags);
		//ConsoleLog(ret);
		//if (ret == "") ret = NULL;
	...
	/// @function InputTextSplit()
	/// @context ImGui
	/// @return {Undefined}
	static InputTextSplit = function(title, val1, val2, label = "##hidden", space = 100, def = NO_DEFAULT, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
	...
	/// @function InputInt3Custom()
	/// @context ImGui
	/// @return {Undefined}
	static InputInt3Custom = function(title, val, label = "##hidden", space = 100, def = NULL, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
		...
		// Input Text
		var ret = ImGui.InputInt3(label, val, flags);
		//if (ret == "") ret = NULL;
	...
	/// @function DragFloat3Custom()
	/// @context ImGui
	/// @return {Undefined}
	static DragFloat3Custom = function(title, val, v_spd = 1, v_min = 0, v_max = 1, label = "##hidden", space = 100, def = NULL, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
		...
		// Input Text
		var ret = ImGui.DragFloat3(label, val, v_spd, v_min, v_max, "%.3f", flags);
		//if (ret == "") ret = NULL;
	...
	/// @context ImGui
	/// @return {Undefined}
	static InputFileCustom = function(title, val, label = "##hidden", space = 100, def = NULL, filter = "", fname = "", directory = "", type = ImGuiInputFileFlags.Directory, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
		...
		if (ImGui.Button("...", 32))
		{
			...
			switch(type)
			{
				case ImGuiInputFileFlags.Directory:
					file = get_save_filename_ext(filter, fname, directory, title);
					if (file != "" && ord(file) != 0) ret = filename_path(file);
					break;
				case ImGuiInputFileFlags.Save:
					file = get_save_filename_ext(filter, fname, directory, title);
					if (file != "" && ord(file) != 0) ret = file;
					break;
				case ImGuiInputFileFlags.Open:
					file = get_open_filename_ext(filter, fname, directory, title);
					if (file != "" && ord(file) != 0) ret = file;
					break;
				default:
					ret = ret;
	...
	/// @context ImGui
	/// @return {Undefined}
	static ComboBoxCustom = function(title, selected, items, label = "##hidden", space = 100, def = NULL, flags = ImGuiInputTextFlags.None)
	{
		// Get Cursor Y
		var ypos = ImGui.GetCursorPosY();
		var ret = selected;
		...
		// Input Text
		if (ImGui.BeginCombo(label, selected != -1 ? items[selected] : "---", ImGuiComboFlags.None))
		{
	...
	/// @context ImGui
	/// @return {Undefined}
	static DragFloatCustom = function(title, val, label = "##hidden", v_speed = 1, v_min = 0, v_max = 0, space = 100, def = NULL, format = "%.3f", flags = ImGuiInputTextFlags.None)
	{
		var ypos = ImGui.GetCursorPosY();
		ImGui.Text("");
		ImGui.SameLine(space);
	...
	static DetailSplit = function(title = "##", value = 0, xx = 0, width = 0)
	{
		ImGui.SetCursorPos(xx, ImGui.GetCursorPosY() - 2);
		ImGui.PushItemWidth(floor(width / 2) - 4);
		ImGui.InputText(title + "Dec", value, ImGuiInputTextFlags.ReadOnly);
		ImGui.SameLine();
		ImGui.SetCursorPos(xx + floor(width / 2) - 2, ImGui.GetCursorPosY() - 2);
		ImGui.PushItemWidth(floor(width / 2) - 5);
		ImGui.InputText(title + "Hex", "0x" + string_hex(value, 4), ImGuiInputTextFlags.ReadOnly);
	}
	
	static Detail = function(title = "##", value = 0, xx = 0, width = 0)
	{
		ImGui.SetCursorPos(xx, ImGui.GetCursorPosY() - 2);
